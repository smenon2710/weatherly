...
**Shipped, then refined same day after real-use feedback.** See `CLAUDE.md`'s "Glanceable AI ring + tap-to-expand summary" section and `IMPROVEMENTS.md` for full detail. Summary: `WeatherData.pressureDropAlert` (a real ≥3 hPa/6h drop signal) drives a pulsing ring behind the hero temperature, and the existing headline pill is now tappable, opening a new `DetailSheet.Forecast` sheet. **Follow-up fix (user-reported the sheet was redundant with the pill):** the sheet now always shows a plain "Pressure: X hPa, falling/rising/holding steady" line (`WeatherData.pressureTrend6h`) — not just on the rarer alert threshold — so it has genuinely new content every time, not just a repeat of the pill's sentence. Verified live on-device: sheet reads "Pressure: 1005 hPa, holding steady" for a real no-anomaly forecast; the ring itself still isn't visually confirmed since no genuine pressure-drop anomaly has occurred during any testing session yet.
...
**Shipped, then refined twice same day after real-use feedback.** See `CLAUDE.md`'s "Fluid day-detail transition" section and `IMPROVEMENTS.md` for full detail. Summary: tapping a day in the 7-day forecast now grows that row into a full-screen detail view via Compose's `SharedTransitionLayout`/`AnimatedContent`, replacing the old `DetailSheet.Day` bottom sheet (removed entirely). **Follow-up fix #1 (user-reported a visible flash):** `AnimatedContent` was defaulting to a full-screen cross-fade of the outgoing and incoming content — fixed with `transitionSpec = { EnterTransition.None togetherWith ExitTransition.None }` plus `resizeMode = scaleToBounds()` on both `sharedBounds` calls, so only the shared row/header element visibly animates. **Follow-up fix #2 (user-reported ~35% empty space, asked if real data could fill it):** added `DayEntry.dayHourly` — a per-day hourly breakdown using Open-Meteo data that was already fetched but previously discarded — rendered as a new scrollable strip. Consistency with the main screen's existing "Next 24 Hours" strip is guaranteed by construction (one shared `hourEntryAt()` function feeds both), verified via a direct side-by-side comparison of 5 overlapping hours, all matching exactly. Verified live on-device across two different days, back-navigation preserving scroll position, no transition ghosting, and the hourly-value consistency check.
