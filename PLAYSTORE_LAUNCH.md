...
- [ ] **Next upload:** versionCode 7 / 1.0.6 — bundles the Settings screen (items 25/35/36), a security fix for the OpenRouter key field (B9), and four layout bugs found via tester feedback and on-device testing (B6–B8, plus two header-crowding rounds). Build the signed AAB and upload to the same `skyspeak-testers` track — this does not reset the 14-day tester-continuity clock, only a drop below 20 continuously-opted-in testers would
