...
- [x] **versionCode 10 / 1.0.9 — built and verified locally (2026-07-17); not yet uploaded to Play Console.** Ships the Alert Display Redesign (B18 in `IMPROVEMENTS.md`): `AlertBannerList`/`ResolvedAlertCard` shrink to a single-line severity strip, `DetailSheet.AlertList` becomes a tappable summary-row chooser instead of stacking full alert content, and `DetailSheetContent` gains a `verticalScroll` safety net so no sheet type can silently crop content again.
  ...
  - [ ] Upload `app-release.aab` to Play Console → Closed Testing (`skyspeak-testers` track) — does not reset the 14-day tester clock (same as versionCode 6→7→8→9)
