...
- [ ] **Holding on Production intentionally** — user is monitoring Closed Testing for a period before promoting, rather than going straight through the same day (unlike versionCode 13's same-day Internal → Closed → Production). Once ready: promote via Play Console → Production → Promote release → select versionCode 14, then watch Android Vitals over the following days per the usual process.
