...
- [ ] Not yet uploaded to any track. Cache reuse, the breakpoint downgrade, and the three real-device bugs above are all fixed and verified; no-stale-while-revalidate is the one remaining known issue, and the MORNING/NIGHT focus variants plus the manual-refresh tap behavior remain unverified on a real device. Given no-stale-while-revalidate is a bounded cold-start-only gap rather than an ongoing/every-placement defect, it's a judgment call whether to fix it before this release or ship and track separately. Once a decision is made: Internal Testing first (per the corrected sequencing), then promote to Closed Testing, then Production.
