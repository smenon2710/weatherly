...
- [ ] Not yet uploaded to any track. Cache reuse and the breakpoint downgrade are both fixed and verified; no-stale-while-revalidate is the one remaining known issue, and the MORNING/NIGHT focus variants remain visually unverified (real-device follow-up). Given the remaining issue is a bounded cold-start-only gap rather than an ongoing/every-placement defect, it's a judgment call whether to fix it before this release or ship and track separately — not automatically a blocker the way the breakpoint downgrade was. Once a decision is made: Internal Testing first (per the corrected sequencing), then promote to Closed Testing, then Production.
