...
| Phone screenshots | 2–8, min 1080px tall, ≤2:1 aspect ratio, no alpha | ⚠️ **Stale as of 2026-07-17** — `store_assets/screenshot-0{1,2,3}-*.png` are 1280×2560 (exact 2:1), alpha flattened, but `-01-weather` (old static gradient hero) and `-03-radar` (screen no longer exists) both need re-shooting against the current build. `-02-chat` is unaffected. |
...
- [ ] **versionCode 9 / 1.0.8 bumped in `build.gradle.kts` (2026-07-17), not yet built/uploaded.** Removes the radar map (low perceived value, plus its `osmdroid` dependency/permission/ProGuard rule) and replaces it with a full-screen animated weather background (26 real-data-driven conditions). Also fixes real rain/snow-accuracy bugs: `WeatherAdvisor.umbrella()` could recommend an umbrella on snow days; the Precipitation tile/chat context/detail chart didn't distinguish rain from snow at all; `SparklineTile` auto-scaled percentage charts so a 10–25% day and an 80–100% day looked visually identical. Still to do before this is actually "shipped" to testers:
  - [ ] Build the signed AAB and upload to the Closed Testing track (does not reset the clock, per the established pattern above)
  - [ ] Re-shoot `store_assets/screenshot-01-weather.png` (old static gradient hero → new animated background) and `screenshot-03-radar.png` (shows a screen that no longer exists — must be replaced, not left as-is). `screenshot-02-chat.png` is unaffected and can stay
  - [ ] Re-upload the replacement screenshots to Play Console → Store listing
  - [ ] Paste the corrected short/full description (already rewritten below in this doc) into Play Console → Store listing — Console has its own copy independent of this file
