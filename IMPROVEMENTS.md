...
| — | UI: `AlertBannerList` (severity-colored `GlassCard` above the hero — see the Design Audit follow-up below for why this isn't full-bleed/square as first shipped), "+N more" affordance instead of stacking every alert, `DetailSheet.Alert`/`AlertList` + `AlertDetailContent` for the full text |
