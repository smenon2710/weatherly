...
**Still open, separate, and not addressed here:** the `Column container cannot have more than 10 elements` error still fires on every widget update after this fix (confirmed in the same regression sweep's logcat) — it just no longer appears to affect visible output at any tested size. Given it's real, logged at `E` level, and its actual source composable/container is still unidentified, it's worth a dedicated future investigation, but doesn't block this fix and isn't currently known to cause any user-visible symptom.
