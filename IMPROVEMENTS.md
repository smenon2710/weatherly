...
## Completed — Adaptive Haptic Feedback (2026-09-04)

First item implemented from `PREMIUM_UX_ROADMAP.md`'s Easy tier (the only item left there after Semantic Lifestyle Context was considered and dropped for a liability reason — see that doc's "Considered and dropped" section).

**What shipped:** a single, restrained haptic pulse when a fresh forecast loads for a genuinely notable condition, via new `util/WeatherHaptics.kt`:
- An active severe/extreme NWS alert → the strongest pattern (three sharp, escalating pulses).
- A thunderstorm (WMO 95/96/99) → an irregular, uneven-timing pattern meant to read as a distant rumble rather than a mechanical buzz.
- Heavy rain, heavy snow, or violent showers (WMO 65/75/82 — the exact codes `util/WeatherIcon.kt`'s `wmoText()` already labels this way, reused rather than re-derived) → a few light, evenly-spaced clicks, a "pitter-patter" impression.
- Everything else (the common case — clear, partly cloudy, ordinary light rain, etc.) stays silent by design. This is deliberately narrower than the 26-scene visual system in `WeatherBackground` — the goal is "felt occasionally for something worth noticing," not a buzz on every screen open.

**Trigger point, deliberately restrictive:** `WeatherViewModel.load()` only fires the haptic on a **foreground** load — cold start, an error retry, or a location/unit change (`background = false`) — never the silent 30-minute periodic refresh or a pull-to-refresh (`background = true`), both of which can fire while the user isn't looking at the phone at all. Firing on those would be a real, unwanted surprise vibration, not a "premium touch."

**Settings toggle:** Settings → Haptic Feedback, on/off `OptionPill`s, default on — same UI pattern as the existing Widget Background toggle. `PreferencesStore.getHapticFeedbackEnabled()`/`setHapticFeedbackEnabled()` back it, mirroring every other boolean preference in that class.

**Technical notes:** `VibrationEffect.createWaveform()` with an amplitude array (real intensity control, not just on/off buzzing) has existed since API 26 — this app's own `minSdk`, so no version-gated fallback was needed for the effect itself, only for obtaining the `Vibrator` (`VibratorManager` on API 31+, the deprecated `Context.VIBRATOR_SERVICE` below that — a small, standard two-branch helper). `VIBRATE` is a normal permission (no runtime prompt, no Data Safety disclosure change) — added to `AndroidManifest.xml`.

**Verification:** `assembleDebug`, `test` (38/38 passing — the haptics change touches no code the existing `WeatherAdvisorTest` suite exercises), and `lint` all green under AGP 9.4.0/Gradle 9.6.0 (the one flagged `prefs.edit()` KTX-extension suggestion is a pre-existing style note that already applies identically to all 11 other `prefs.edit()` calls in `PreferencesStore.kt`, not something newly introduced). `aapt2 dump permissions` confirmed `android.permission.VIBRATE` present in the built debug APK. **Not yet verified on a real device** — no device/emulator was available in this session, so the actual felt pattern timings (rumble irregularity, click spacing, overall intensity) are a first pass based on reasoning about `VibrationEffect` parameters, not a user-tested feel. Worth a real on-device check (does the thunder pattern actually read as a "rumble," is the severe-alert pattern too strong/too weak, does the trigger point feel right in practice) before considering this genuinely done, same discipline every other feature in this app gets before shipping.

---

## Completed — Stale Daily "Chance of Rain" Fix (2026-09-04)

Found via a real-device sanity check the same session as the haptics feature above — not inspection-driven. User-reported: the 7-day forecast showed "40% chance of rain today," the hero tip read "Rain likely today — grab an umbrella before you head out," and the day's own detail sheet repeated the same "Rain likely" advice — but the Precipitation detail chart showed almost nothing for the rest of the day, and the chance read as "very very low."

**Root cause, confirmed by pulling the actual cached `WeatherData` off the reporting device** (`adb run-as ... cat shared_prefs/forecast_cache.xml`, Franklin Township, NJ, 4:14 PM local): Open-Meteo's daily `precipitation_probability_max` and its dominant daily `weathercode` both describe the **full calendar day (00:00–23:59)** — a real number, but one that can describe a window that's already elapsed. Today's daily entry showed `precipProbMax: 40` and dominant code 51 ("Light drizzle"), almost certainly from earlier that morning; the app's own forward-looking hourly data (which starts at "now") showed chance topping out at 17% and `hourlyPrecipAmount: 0.0` for literally every one of the next 24 hours, with a whole-day rain total of just 0.4mm. `buildTips()` and `buildDayOutlookTips()`'s `isRain = code in 51..67 || ...` check fires unconditionally off that dominant code — independent of how low the actual probability is — which is why "Rain likely" showed regardless of the (already low, and now confirmed further overstated) percentage next to it.

**Fix — corrected at the data source in `WeatherRepository.kt`'s `mapToWeatherData()`, not at each display site**, so every consumer of the field benefits automatically rather than needing separate patches:

1. **`DayEntry.precipProbMax` for today's own entry only** (`i == todayIndex` in the `daily` build loop) is now the max hourly `precipChance` across the hours still remaining today — bounded by the first `"12 AM"` hour label, reusing the exact same boundary `MetricChart.dayChangeIndex` already relies on elsewhere, rather than a new mechanism. Every other day in the list is untouched: they haven't started yet, so Open-Meteo's own full-day aggregate is exactly the right thing to show for them.
2. **`buildDayOutlookTips()`'s code input for today's own entry** is the live `currentCode` (what's actually happening right now) instead of today's historical dominant code — otherwise the corrected, now-honest percentage would still sit next to an unconditional "Rain likely" tip driven by the old code, which was the first thing checked after the initial fix and found still broken (17% next to "Rain likely — pack an umbrella," a *more* visibly inconsistent pairing than the original 40%). Every other day keeps using its own dominant code, correctly, since those days haven't started.
3. **The hero `WeatherData.tips` banner** (`buildTips()`'s current-day call) gets the same `currentCode`-over-historical-code treatment, plus a `tipsPop` sourced from the now-corrected `today.precipProbMax` directly rather than a separate fixed 12-hour lookahead — removing a small duplication between the two fixes.
4. **Deliberately left alone:** `DayEntry.icon`/`phrase` (the WMO glyph and "Light drizzle" label shown in the 7-day list itself) — recomputing a single "dominant condition for the remaining hours" is a fuzzier problem (today's remaining hours were a mix of clear and overcast; which should represent the row?) with more edge cases for less payoff than the number/tip-text fix, since the core confusion was the percentage-plus-advice-text combination, not the small icon. The night-rollover case (tips describing tomorrow once today's day is done) is also untouched — tomorrow's own full-day aggregate is legitimate there since tomorrow hasn't started yet.

**Free side effects, not separately implemented:** because the fix lives on `DayEntry.precipProbMax` itself, `WeatherAdvisor`'s umbrella-type advice (`daily.firstOrNull()?.precipProbMax`), `ChatRepository`'s AI weather-context brief (same field), and `WeatherWidget`'s home-screen rain percentage (same field) are all corrected automatically — none of those files were touched.

**Verified live, twice, against the same real conditions** (cache cleared, app force-stopped and cold-launched, `forecast_cache.xml` re-inspected after each fresh network fetch):
- First pass fixed the hero tip and the percentage (40% → 17%) but left the day-detail-sheet tip still reading "Rain likely" — caught immediately by re-checking the actual JSON rather than assuming the fix was complete.
- Second pass (the `currentCode`-for-`buildDayOutlookTips()` change) confirmed all three now agree: hero tip *"No major weather to plan around today,"* 7-day list **17%**, and the day-detail-sheet tip *"No major weather concerns for this day."* Tomorrow's entry (5%, its own untouched tips) confirmed unaffected, as intended.
- One transient `WeatherRepository: getWeather failed` warning was caught by an active logcat monitor during testing; investigated rather than ignored — an immediate retry with byte-identical code succeeded cleanly, and the new logic is a pure computation over already-fetched data with no location/network dependency, so a real bug in it would be expected to reproduce deterministically rather than intermittently. Read as a one-off location-fix timing hiccup from force-killing and immediately cold-relaunching the app during testing, not a regression from this change — flagged here rather than silently dismissed.
- `assembleDebug`, `test` (38/38), and `lint` all green throughout both passes.

---

## Completed — AQI/Pressure Tile Height Mismatch (2026-09-04)

User-reported real-device observation: the Pressure tile in the "Atmosphere" row looked visually different (shorter) than the Air Quality tile beside it. Root cause: Air Quality carries a `MetricTileData.advisory` line (e.g. "OK for most people") that Pressure never does (see CLAUDE.md's advisory-field write-up — Pressure is one of the tiles deliberately left without one), so `ArcGaugeTile`'s extra advisory `Spacer`+`Text` made AQI's `GlassCard` taller, with nothing forcing Pressure's card to match. The "Sky: Sunrise + Visibility" row directly below already solves this exact class of problem with `Modifier.height(IntrinsicSize.Max)` on the `Row` plus `.fillMaxHeight()` on both children — the "Atmosphere: AQI + Pressure" row above it simply never got the same treatment. Applied the identical, already-established pattern rather than inventing a new one. Verified with a real on-device screenshot (both cards now match exactly) and `assembleDebug`/`test` (38/38)/`lint` all green.

---

## Completed — 7-Day List Precipitation Badge: Threshold Instead of Icon Gate (2026-09-04)

Found the same session as the stale daily-tip fix above, via the same kind of real-device sanity check: user noticed the day-detail sheet showed real, nonzero "Chance of rain" percentages for several upcoming days (5–10% range) that never appeared as a badge in the 7-day list itself. Root cause: `DailyCard`'s inline percentage (`WeatherComponents.kt`) was gated on `precipProbMax > 0 && day.icon in 45..99` — requiring the day's *own dominant WMO code* to already be a precipitation/fog code before showing any number at all, while `DetailSheet.Day`'s "Chance of rain" `DetailRow` shows the raw number unconditionally. Since a day's code and its actual probability aren't tightly correlated (an Overcast day, code 3, can carry any real percentage and never pass the gate; a technically-precip-coded day passes regardless of how low its number is), the two views could — and did — visibly disagree.

**Fix, informed by the user's own research into meteorological PoP conventions** (30%/40%/60% as the "cautionary/balanced/conservative" industry thresholds for when a rain icon should appear): replaced the icon gate with `precipProbMax >= 40` — the cited "balanced" standard, chosen over 30% (too permissive, reintroduces clutter) or 60% (too conservative, would hide genuinely-worth-a-glance days) — and dropped the icon condition entirely. This creates a coherent two-tier hierarchy with the existing `buildTips()`/`buildDayOutlookTips()` threshold: **≥40% earns a quiet number in the list, ≥50% earns the bolder "Rain likely — grab an umbrella" text.** The detail sheet's `DetailRow` is intentionally left unconditional — it's the full-info view, always showing the true number, while the list is deliberately a highlights view.

**Verified live on-device via screenshot after the fix:** none of Sat/Sun/Mon/Tue/Wed/Thu (real values 5%/7%/3%/0%/10%/12%) show a badge, confirming the new threshold works — notably, Thursday (12%, drizzle-coded) previously *did* show a badge under the old icon gate and now correctly doesn't, since 12% doesn't clear the meaningful bar either way. `assembleDebug`, `test` (38/38), and `lint` all green.

---

## Completed — Icon Intensity/Hail Differentiation from Existing WMO Data (2026-09-04)

User asked how many weather icons the app has, then followed up citing broadcast-TV icon vendors (mikeafford.com, metgraphics.net) whose standard sets run to 92+ distinct icons. Investigated rather than just answering the count: `wmoText()`'s exhaustive `when` confirmed Open-Meteo returns exactly 28 distinct WMO codes — a hard data ceiling those 92-icon broadcast sets don't share, since those exist for a *human* meteorologist to hand-pick from, not a machine-driven classification. But investigating further surfaced a real, actionable gap: Open-Meteo's own 28-code table already encodes slight/moderate/heavy (or slight/heavy) severity as *distinct codes* for drizzle (51/53/55), rain (61/63/65), freezing drizzle (56/57) and freezing rain (66/67), snow (71/73/75), showers (80/81/82, 85/86), and hail (95 vs 96/99) — and `WeatherBackground.classify()`'s animated scene already uses exactly this distinction (`RainIntensity`/`SnowIntensity` tiers, `THUNDER` vs `THUNDER_HAIL`). `WeatherGlyph.kt`'s compact static icon set, used everywhere else (hourly row, daily row, hero, chat, and the widget), was throwing all of that away — every code in a RAIN or SNOW family rendered as an identical fixed-count icon regardless of real severity.

**Fix:** two new pure functions in `WeatherGlyph.kt`, both derived only from the `code: Int` every call site already passes in — no new data fields, no call-site changes needed anywhere:
- `intensityFor(code): Int` (1–3) — light/moderate/heavy per the code's own WMO tier.
- `hasHail(code): Boolean` — true for 96/99.

`drawWeather()`'s RAIN/SNOW/MOON_RAIN/MOON_SNOW branches gained a `dropOffsetsFor(intensity, full)` helper that selects 1, 2, or all 3 of each shape's *existing* fixed drop/flake positions (no new positions invented, just fewer/more of the same ones) — a light-tier code draws one drop, moderate draws two, heavy draws all three. `THUNDER`/`MOON_THUNDER` gained a small `drawHail()` accent (two `colors.snow`-tinted dots flanking the bolt) shown only when `hasHail(code)` is true, reusing the existing snow color rather than adding a new palette field. The one true entry point (`WeatherGlyph()` composable) computes both from `code` automatically, so all five call sites (`ChatScreen`, `CurrentHeader`, `HourlyCard`, `DailyCard`, `DetailSheet.Day`) and the widget's `renderGlyphBitmap` (updated to pass the same two values through) all pick up the fix for free.

**Deliberately out of scope:** cloud-cover/wind-based icon variation (e.g., a "mostly cloudy" vs "partly cloudy" shape distinction mirroring `WeatherBackground`'s `cloudCoverPct` split) — that would need new data plumbing through every call site (none of `HourEntry`, the chat/hero paths currently pass cloud cover to the icon layer) for a distinction likely imperceptible at the 16–48dp render sizes these icons actually use, unlike drop-count which is clearly visible even small. Flagged as a possible future pass, not pursued here.

**Verified live on-device via screenshot:** a "Light drizzle" (WMO 51) day in the 7-day list — both "Today" and "Thu" in the test session — now renders with a single raindrop instead of the previous fixed three. `assembleDebug`, `test` (38/38), and `lint` all green.

---

## Completed — Glanceable AI Ring + Tap-to-Expand Summary (2026-09-04)

First of two `PREMIUM_UX_ROADMAP.md` Medium-tier items implemented in one session (the second, Fluid Layout Transitions, follows immediately below). User asked to implement both after narrowing Medium tier down to these two and shelving gyroscope particles/voice.

**What shipped:**
- `WeatherRepository.mapToWeatherData()` computes `pressureDropAlert: Boolean` — a sustained drop of ≥3 hPa between the current hour's pressure (`hourlyPressure[0]`) and the lowest value across the next 6 hourly entries. A real, recognized meteorological signal that conditions may be changing, computed entirely from data already fetched (`hourlyPressure`), no new API fields.
- `CurrentHeader` (`WeatherComponents.kt`) draws a subtle pulsing ring (`Cyan`, alpha 0.15↔0.45, 1.4s cycle — same `rememberInfiniteTransition` pattern `SparklineTile`'s existing "Now" dot pulse already uses) behind the hero temperature, but only when `pressureDropAlert` is true. The common no-anomaly case renders the temperature exactly as it did before this feature — verified by leaving the non-anomaly code path completely untouched (an `if/else` split, not a conditional modifier on shared code).
- The existing headline pill (`WeatherData.headline`, already shown under the hero) is now clickable, opening a new `DetailSheet.Forecast(headline, pressureDropAlert)` sheet via the existing `ModalBottomSheet`/`DetailSheetContent` machinery — no new sheet mechanism introduced, reusing exactly what `DetailSheet.Metric`/`Day`/`Alert`/`AlertList` already use. The sheet shows an `Icons.Filled.AutoAwesome` sparkle in `Cyan` (matching the chat FAB's existing icon/color, so it reads as "the same AI" rather than a new visual language) and the headline at a larger size than the compact pill; when `pressureDropAlert` is true, an extra plain-data sentence about the pressure trend is appended (deliberately not framed as advice — see the "Considered and dropped" lifestyle-context entry for why suitability-flavored language was ruled out for this exact kind of proactive surface).

**Verified live on-device:** installed on the reporting user's Pixel 9 Pro, launched fresh, tapped the headline pill, and confirmed the new "Forecast Insight" sheet opens with the correct sparkle icon, title, and headline text ("Mixed conditions over the next few hours."). The ring itself wasn't visually confirmed — real conditions during testing had no genuine ≥3 hPa/6h pressure drop, so the ring correctly never rendered; the no-anomaly code path was what got exercised. `assembleDebug`, `test` (38/38), and `lint` all green.

---

## Completed — Fluid Day-Detail Transition (2026-09-04)

Second of the two `PREMIUM_UX_ROADMAP.md` Medium-tier items implemented this session (the AI ring above shipped first). Genuinely bigger architectural change than most items this session — a real Compose `SharedTransitionLayout` integration, not a modifier tweak.

**What shipped:** tapping a day in the 7-day forecast (`DailyCard`) now grows that specific row into a full-screen detail view via a shared-element transition, instead of `DetailSheet.Day` sliding up as a `ModalBottomSheet`. `DetailSheet.Day` was removed entirely — its only construction site was `DailyCard`'s own tap handler, so once that switched to the new mechanism it became genuinely dead, not left behind as an unused code path.

**How it's built:**
- `WeatherContent` now holds `expandedDay: DayEntry?` and wraps its entire body in `SharedTransitionLayout { AnimatedContent(targetState = expandedDay) { day -> ... } }`. The existing screen content was extracted into a new private `WeatherContentBody` composable (content unchanged, just parameterized to take `sheet`/`onSheetChange` as explicit params instead of owning that state directly, since `WeatherContent` is no longer the sole state holder) and rendered in the `null` branch; a new `ExpandedDayDetail` composable renders in the non-null branch.
- `DailyCard`'s tapped `Row` and `ExpandedDayDetail`'s root `Column` both carry `Modifier.sharedBounds(rememberSharedContentState(key = "day-${day.dayLabel}"), animatedVisibilityScope)` with the identical key — that's what makes the row visually grow into the full-screen container rather than the new screen just appearing. `dayLabel` ("Today"/"Sat"/.../"Thu") is unique within one 7-day fetch, so no collision risk.
- `DailyCard` became a `SharedTransitionScope` extension function taking an explicit `animatedVisibilityScope: AnimatedVisibilityScope` parameter. Kotlin doesn't support two receivers on one function without opting into the experimental context-receivers compiler feature, which this project doesn't use — so both scopes are threaded through explicitly (`with(this@SharedTransitionLayout) { DailyCard(..., animatedVisibilityScope = this@AnimatedContent) }`) rather than reaching for that flag.
- The actual day content (date, glyph, phrase, H/L, tips, `DetailRow`s) was extracted into a shared `DayDetailBody(day, windUnit, precipUnit)` composable in `WeatherComponents.kt`, so it isn't duplicated between the old sheet path (now gone) and the new full-screen path.
- `ExpandedDayDetail` adds its own back arrow and a `BackHandler(enabled = expandedDay != null) { expandedDay = null }`, so the system back button/gesture closes the expanded view correctly instead of leaving the screen.
- Every other detail sheet (Metric, Alert, AlertList, the new Forecast sheet from the AI-ring feature above) is completely untouched — still the ordinary `ModalBottomSheet` path. This redesign is scoped to exactly the one interaction the roadmap called out, not applied app-wide.

**Compile-time snag caught and fixed:** the first build failed with "this annotation is not repeatable" — `WeatherContent` already carried `@OptIn(ExperimentalMaterial3Api::class)` for its existing `ModalBottomSheet` usage, and stacking a second separate `@OptIn(ExperimentalSharedTransitionApi::class)` annotation isn't valid Kotlin. Fixed by merging into one `@OptIn(ExperimentalMaterial3Api::class, ExperimentalSharedTransitionApi::class)`, and adding the same combined opt-in to the newly-extracted `WeatherContentBody` (which inherited the `ModalBottomSheet` code when it was pulled out of `WeatherContent`).

**Verified live on-device, both directions:** tapped two different days ("Today" and "Saturday, Sep 5") and confirmed each grew into its own full-screen detail with correct, distinct data (right date, condition, H/L, tips, and DetailRows for each). Tapped the back arrow and confirmed it returns cleanly to the 7-day list at its prior scroll position — `WeatherContentBody`'s `rememberScrollState()` survived the `AnimatedContent` transition rather than resetting. One screenshot taken ~1 second after a tap briefly showed overlapping text from the outgoing and incoming content (a transition caught mid-flight); a second screenshot ~2 seconds later showed the same view fully settled and clean — read as a screenshot-timing artifact, not a persistent rendering bug, though this should get a closer look on a future pass if anyone reports a real visible flicker during actual use rather than only during scripted testing. `assembleDebug`, `test` (38/38), and `lint` all green throughout.

---

## Completed — Same-Day Fixes: Transition Flash + Redundant Forecast Sheet (2026-09-04)

Real user feedback on the two features that shipped earlier the same session, from actual use rather than scripted testing — exactly the "overlapping text" this doc's own previous entry had guessed was just a screenshot-timing artifact. It wasn't; investigated properly this time.

**1. The transition flash was real, not a screenshot artifact.** `AnimatedContent` had no custom `transitionSpec`, so it defaulted to cross-fading the entire outgoing screen with the entire incoming screen simultaneously — both full screens' text visibly blended for the transition's duration, on a real device, not just caught mid-frame by a screenshot. Fixed with `transitionSpec = { EnterTransition.None togetherWith ExitTransition.None }` so only the `sharedBounds`-tagged row/header element visibly animates. Separately, neither `sharedBounds` call set a `resizeMode`, so both defaulted to `RemeasureToBounds()` (continuous relayout at every intermediate size during the animation) — janky given the row's compact layout and the destination's full layout share no structure. Fixed with `resizeMode = SharedTransitionScope.ResizeMode.scaleToBounds()` on both.

**Real build snag while fixing this:** the API is `scaleToBounds()` (lowercase) in the Compose Animation version this project actually resolves (1.11.4, via the compose-bom) — confirmed by decompiling the resolved `animation-android-1.11.4.aar`'s `SharedTransitionScope$ResizeMode$Companion` class directly (`javap`) after the capitalized `ScaleToBounds()` failed to compile with "unresolved reference." A different, older version of the same library (1.7.6, also sitting in the local Gradle cache from some earlier resolution) has the capitalized name — the method was renamed between versions, and guessing from stale documentation/memory would have kept failing. Worth remembering: when an androidx API "unresolved reference" doesn't match what you'd expect from a version you recall, check what's *actually* resolved (`./gradlew app:dependencies`) before assuming the API name is simply wrong.

**2. The Forecast Insight sheet was genuinely redundant, user-reported.** In the common (no pressure-drop anomaly) case, the sheet's only content was the identical one sentence the pill already showed, just in a bigger font — no actual reason to tap it. Fixed by adding `WeatherData.pressureTrend6h: Int?` (signed hPa delta between now and 6 hours out, from the same `hourlyPressure` array `pressureDropAlert` already reads) and always showing a plain "Pressure: X hPa, falling/rising/holding steady" line — not gated behind the rarer alert threshold, so there's real content beyond the headline every single time the sheet opens. This is data not shown anywhere else in the app (the Pressure metric tile has the current value but no trend direction).

**Not yet addressed, flagged rather than silently ignored:** the reporting user also noted `ExpandedDayDetail` leaves roughly a third of the screen empty for a typical day (its content — date, glyph, one tip, six `DetailRow`s — rarely fills a full phone screen at `Modifier.fillMaxSize()`), and asked whether real Open-Meteo data exists to fill that space usefully (e.g. an hourly breakdown for that specific day). It does — Open-Meteo's hourly response covers all 7 days, but `WeatherRepository` currently only maps the first 24 hours from "now" into `HourEntry`, discarding the rest. Adding a genuine per-day hourly slice is real new data-layer work (not a bug fix), scoped separately rather than bundled into this fix — see the next conversation for whether/how to pursue it.

**Verified live on-device after both fixes:** Forecast Insight sheet now reads "Pressure: 1005 hPa, holding steady" for a real no-anomaly forecast; a day-tap transition screenshot at the same ~1-second timing that previously showed ghosting now shows a clean, single, correctly-detailed view with no overlap. `assembleDebug`, `test` (38/38), and `lint` all green.

---

## Completed — Per-Day Hourly Breakdown, Guaranteed Consistent with the Main Screen (2026-09-04)

Follow-up to the fixes above, same session: the deferred "empty space" item. User explicitly required that wherever the new per-day hourly data overlaps with the main screen's existing "Next 24 Hours" strip (Today's remaining hours, and potentially a next day if that 24-hour window reaches it), the two must show identical values — no independent recomputation that could quietly drift, especially at the boundary.

**What shipped:** `DayEntry.dayHourly: List<HourEntry>` — each day's own full 00:00–23:00 local hourly breakdown, built from Open-Meteo data that was already being fetched (all 7 days' worth) but previously discarded past the first 24 hours from "now". Rendered via a new `DayHourlyStrip` composable in `DayDetailBody`, placed before the summary `DetailRow`s — real content filling the space that was previously just blank.
