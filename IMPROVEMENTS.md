...
Both parts verified by re-reading the full diff against each reported symptom and design requirement; not yet confirmed on-device (no screenshot from this round). **Not yet released:** uncommitted in the working tree as of this writing — `versionCode` is still 9 (1.0.8, the build currently in Closed Testing); this will ship as versionCode 10 whenever it's next built and uploaded.
