...
**Lookahead pill (`WeatherData.headline`):** Populated by `WeatherRepository.buildUpcomingHeadline()`. Scans the next 12 hourly slots (skipping "Now") for the first significant condition change — Thunderstorm, Snow, Rain, Freezing rain, Drizzle, Fog — relative to the current WMO code. Appends a wind note when the next-12-hour max exceeds 40 km/h / 25 mph. Trusts WMO codes directly; only light drizzle (51–57) requires a ≥30% precipitation-probability floor. Falls back to `comparedToYesterday` text when null.
