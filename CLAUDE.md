...
UI: `AlertBannerList` (`ui/components/WeatherComponents.kt`) renders only the most-severe active alert, built on the same `GlassCard` every other surface in the app uses (same corner radius, shadow, and border treatment per theme, same 16dp side margin as `HourlyCard`/`DailyCard` below it) with `fill` overridden to the severity color — urgency comes from color and top-of-screen position alone, not from breaking the app's shape language. An earlier version was full-bleed and square to read as a "system notice," but that broke consistency with the rest of the app badly enough to read as foreign rather than intentional; reverted after direct user feedback. A small "NATIONAL WEATHER SERVICE" eyebrow (small-caps, `AccountBalance` icon, tinted with the alert's own fg color) renders above the event title — this self-identifies the card's source at a glance, since the icon+bold-title+subtitle+chevron row below it is, structurally, the same template Android's own system notifications use; without an explicit "this is in-app content from X" label a user could plausibly mistake it for an OS-level notification, which matters more here than in a typical card given it's a safety-relevant message (flagged by user feedback). For the same reason, `WeatherScreen` places the card *after* the "sky·speak" wordmark/icon row rather than before it (inside the hero `Column`, before `CurrentHeader`) — the app's own branding is visible before the alert is, so it reads as content inside the app from the first frame rather than something floating above/outside it. A "+N more" row (divided from the primary alert by a `HorizontalDivider`, both still inside the same card) appears when multiple alerts are active, opening `DetailSheet.AlertList`. Tapping the primary alert or a `DetailSheet.AlertList` row opens `DetailSheet.Alert` → `AlertDetailContent`, which shows severity, `AlertBadge` pills for urgency/certainty (NWS's `"Unknown"` value is treated as absent, not rendered), description, instructions, and the effective/in-effect-until times. `alertColors(severity)` collapses the 5-value enum to 3 visual tiers (Critical = Extreme/Severe, Advisory = Moderate, Info = Minor/Unknown) — Critical is the one genuinely new hue in the app (warm rust/oxblood, not a clinical safety red); Advisory reuses the launcher icon's warm-gold; Info reuses the existing `tipColors()` RAIN tone verbatim.
